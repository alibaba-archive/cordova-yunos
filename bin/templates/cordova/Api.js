...
function Api(platform, platformRootDir, events) {
    ...
    this.locations = {
        root: self.root,
        www: path.join(self.root, 'res/asset'),
        res: path.join(self.root, 'res'),
        platformWww: path.join(self.root, 'platform_www'),
        configXml: path.join(self.root, 'res/default/config.xml'),
        defaultConfigXml: path.join(self.root, 'cordova/defaults.xml'),
        strings: path.join(self.root, 'res/default/strings.json'),
        manifest: path.join(self.root, 'manifest.json'),
...
Api.createPlatform = function (destination, config, options, events) {
    ...
    try {
        ...
        events.emit('error','createPlatform is not callable from the yunos project API.');
        throw(e);
    }
    return result;
};
...
Api.updatePlatform = function (destination, options, events) {
    ...
    try {
        ...
        events.emit('error','updatePlatform is not callable from the yunos project API, you will need to do this manually.');
        throw(e);
    }
    return result;
};
...
Api.prototype.addPlugin = function (plugin, installOptions) {
    var project = YunOSProject.getProjectFile(this.root);
    var self = this;
...
Api.prototype.clean = function(cleanOptions) {
    ...
    return require('./lib/check_reqs').run()
      .then(function () {
          return require('./lib/clean').cleanProject.call(self, cleanOptions);
      })
      .then(function () {
